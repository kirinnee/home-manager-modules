{ config, lib, pkgs, gwsPackage, ... }:

let
  cfg = config.programs.multi-gws;
...
in
{
  options.programs.multi-gws = {
    enable = lib.mkEnableOption "Google Workspace CLI Multi-Account manager";

    defaultPackage = lib.mkOption {
      type = lib.types.package;
      default = gwsPackage;
      description = "Default gws package to use";
    };
    ...
    smartWrapper = {
      enable = lib.mkOption {
        type = lib.types.bool;
        default = true;
        description = "Create smart `gws` wrapper that auto-detects account";
      };
    };
    ...
    shellIntegration = {
      functions = lib.mkOption {
        type = lib.types.bool;
        default = false;
        description = "Create `gws-<name>` shell functions for each account";
      };

      showActive = lib.mkOption {
        type = lib.types.bool;
        default = true;
        description = "Add `_gws_active_account()` function for shell prompts";
      };
    };
    ...
    aliases = lib.mkOption {
      ...
      default = { };
      description = "Alias definitions. Each alias creates a short command that appends flags to gws. For example, `gd = \"drive list\"` creates `gd`, `gd-personal`, `gd-work`, etc.";
    };
      ...
      type = lib.types.attrsOf (lib.types.submodule ({ name, config, ... }: {
        options = {
          ...
          env = lib.mkOption {
            type = lib.types.attrsOf lib.types.str;
            default = { };
            description = "Environment variables to export before running gws binary. Supports shell expansion.";
          };
        };
      }));
      default = { };
  ...
  config = lib.mkIf cfg.enable (
    let
      ...
      wrappedPackages = lib.mapAttrs createWrappedGws enabledAccounts;

      # Generate the smart wrapper script
      smartWrapperScript = pkgs.writeShellScriptBin "gws" ''
        # Google Workspace CLI Multi-Account Smart Wrapper
        # Automatically switches accounts based on current working directory

        ${generateMatchFunction}

        # Main execution
        account=$(_gws_match_account)

        # Show which account is being used
        if [[ -n "$account" ]]; then
          echo "🔐 Using gws account: $account" >&2
        fi
      ...
      assertions = [
        {
          ...
          message = "programs.multi-gws.defaultAccount must reference an existing account. Got '${cfg.defaultAccount}' but available accounts are: ${lib.concatStringsSep ", " accountNames}";
        }
        {
          assertion = accountNames != [ ];
          message = "programs.multi-gws requires at least one account to be defined";
        }
      ];
      ...
      programs.zsh.initContent = lib.mkIf (config.programs.zsh.enable && (cfg.shellIntegration.functions || cfg.shellIntegration.showActive)) (
        let
          ...
          activeFunction = lib.optionalString cfg.shellIntegration.showActive ''
            ...
            _gws_active_account() {
              local account=$(_gws_match_account)
